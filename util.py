...
def read_validation_batch(batch_size, validation_source, annotations, dic_list=None):
    ...
    if dic_list:
        ...
        with open(annotations) as f:
            gt_idxs = f.readlines()
            gt_idxs = [(i, np.where(subclass == (int(x.strip()) - 1))[0][0]) for i, x in enumerate(gt_idxs) if
                       (int(x.strip()) - 1) in subclass]
...
def read_centre_crop(image_path):
    ...
    img_cropped = img.crop(
        ((img.size[0] // 2) - 112, (img.size[1] // 2) - 112, (img.size[0] // 2) + 112, (img.size[1] // 2) + 112))
